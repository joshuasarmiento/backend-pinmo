...
async function uploadProfilePicture(file, userId) {
    try {
        const fileName = `${userId}/${(0, uuid_1.v4)()}-${file.originalname}`;
...
router.post('/forgot-password', async (req, res) => {
    try {
        ...
        const { error: authError } = await supabase_1.supabase.auth.resetPasswordForEmail(email, {
...
router.post('/reset-password', async (req, res) => {
    try {
        ...
        const { error: authError } = await supabase_1.supabase.auth.admin.updateUserById(user.id, {
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (error || !data.user) {
            console.log('Login failed:', error === null || error === void 0 ? void 0 : error.message);
...
router.post('/profile/picture', auth_1.authenticate, upload.single('profile_picture'), async (req, res) => {
    try {
        const userId = req.user.id;
        const file = req.file;
