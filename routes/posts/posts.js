...
const uploadImageToSupabase_1 = require("../../utils/uploadImageToSupabase");
...
router.get('/notifications', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const cached = (0, cache_1.safeGetCache)(cacheKey);
        if (cached) {
        ...
        (0, cache_1.safeSetCache)(cacheKey, result, 60);
...
router.get('/notifications/unread-count', auth_1.authenticate, async (req, res) => {
    try {
        ...
        (0, cache_1.safeSetCache)(cacheKey, unreadCount, 30);
...
router.put('/notifications/:id/read', auth_1.authenticate, (0, cache_1.autoInvalidateCache)(req => `notifications:${req.user.id}`), async (req, res) => {
    try {
        const notificationId = req.params.id;
        const userId = req.user.id;
        console.log('Marking notification as read:', notificationId);
        const { data: notification, error: fetchError } = await supabase_1.supabase
            .from('notifications')
            .select('user_id')
...
router.put('/notifications/read-all', auth_1.authenticate, (0, cache_1.autoInvalidateCache)(req => `notifications:${req.user.id}`), async (req, res) => {
    try {
        const userId = req.user.id;
...
// Create post
router.post('/', auth_1.authenticate, advancedPostValidation_1.advancedRateLimit, advancedPostValidation_1.rateLimit, advancedPostValidation_1.enhancedFileUpload.fields([
    { name: 'images', maxCount: 5 },
    { name: 'custom_pin', maxCount: 1 }
]), advancedPostValidation_1.comprehensiveValidation, (0, cache_1.autoInvalidateCache)(req => 'posts'), async (req, res) => {
    var _a, _b;
    try {
        ...
        console.log('Files received:', {
            images: ((_a = files === null || files === void 0 ? void 0 : files.images) === null || _a === void 0 ? void 0 : _a.length) || 0,
            custom_pin: ((_b = files === null || files === void 0 ? void 0 : files.custom_pin) === null || _b === void 0 ? void 0 : _b.length) || 0
        });
        let imageUrls = [];
        let customPinUrl = null;
        // Handle post images
        if ((files === null || files === void 0 ? void 0 : files.images) && files.images.length > 0) {
            try {
                ...
                imageUrls = await Promise.all(files.images.map(file => (0, uploadImageToSupabase_1.uploadImageToSupabase)(file, 'posts')));
        ...
        if ((files === null || files === void 0 ? void 0 : files.custom_pin) && files.custom_pin.length > 0) {
            try {
                console.log('Uploading custom pin image...');
                customPinUrl = await (0, uploadImageToSupabase_1.uploadImageToSupabase)(files.custom_pin[0], 'custom-pins');
        ...
        const sanitizedType = (0, advancedPostValidation_1.sanitizeForDatabase)(type);
        const sanitizedDescription = (0, advancedPostValidation_1.sanitizeForDatabase)(description);
        const sanitizedLocation = (0, advancedPostValidation_1.sanitizeForDatabase)(location);
        const sanitizedLink = link ? (0, advancedPostValidation_1.sanitizeForDatabase)(link) : null;
        const sanitizedEmoji = emoji ? (0, advancedPostValidation_1.sanitizeForDatabase)(emoji) : null;
...
router.get('/', async (req, res) => {
    ...
    try {
      ...
      `)
        ...
        let postsWithUserAndLikes = await Promise.all(posts.map(async (post) => {
            ...
            if (authUserId) {
                try {
                    const { data: like } = await supabase_1.supabase
                        .from('post_likes')
                        .select('id')
                        .eq('post_id', post.id)
        ...
        (0, cache_1.safeSetCache)(cacheKey, postsWithUserAndLikes, 60);
...
router.get('/:newId', async (req, res) => {
    try {
        ...
        (0, cache_1.safeSetCache)(cacheKey, postWithUser, 300); // Cache for 5 minutes
...
// Update post
router.put('/:id', auth_1.authenticate, (0, cache_1.autoInvalidateCache)(req => 'posts'), async (req, res) => {
    try {
        ...
        (0, cache_1.invalidateUserPostsCache)(userId);
        if (post.new_id) {
...
// Delete post
router.delete('/:id', auth_1.authenticate, (0, cache_1.autoInvalidateCache)(req => 'posts'), async (req, res) => {
    try {
        const postId = req.params.id;
        const userId = req.user.id;
        ...
        if (post.image_url && Array.isArray(post.image_url)) {
            try {
                const deletePromises = post.image_url.map(async (url) => {
                    // Extract file path from URL
                    const urlParts = url.split('/');
                    const fileName = urlParts[urlParts.length - 1];
                    const filePath = `posts/${fileName}`;
                    return supabase_1.supabase.storage
                        .from('pinmo-images')
                        .remove([filePath]);
                });
